...
def get_all_click_sample(data_path, sample_nums=10000):
    ...
    """
    all_click = pd.read_csv(data_path + "train_click_log.csv")[:10000]
...
def get_item_info_df(data_path):
    item_info_df = pd.read_csv(data_path + "articles.csv")[:10000]
...
def get_item_emb_dict(data_path):
    item_emb_df = pd.read_csv(data_path + "articles_emb.csv")[:10000]
...
def gen_data_set(data, negsample=0, test_ratio=0.2, min_hist_len=1):
    """
    改进版数据集生成函数
    :param test_ratio: 测试集比例
    :param min_hist_len: 最小历史长度
    """
    data = data.sort_values("click_timestamp")
    item_ids = data["click_article_id"].unique()
    ...
    for reviewerID, hist in tqdm(data.groupby("user_id")):
        ...
        # 跳过历史过短的用户
        if len(pos_list) < min_hist_len + 1:
            continue
        ...
        # 计算测试集大小
        test_size = max(1, int(len(pos_list) * test_ratio))
        ...
        for i in range(1, len(pos_list)):
            ...
            if is_test:
                # 测试集只包含正样本
                test_set.append(
                    (reviewerID, hist, target, 1, len(hist))  # 保持时间顺序
                )
            else:
                # 训练集: 正样本 + 负样本
                train_set.append((reviewerID, hist, target, 1, len(hist)))

                if negsample > 0:
                    for negi in range(negsample):
                        neg_idx = (i - 1) * negsample + negi
                        train_set.append(
                            (reviewerID, hist, neg_list[neg_idx], 0, len(hist))
                        )
...
def user_based_recommend(
    ...
    """
    # 历史交互
    user_item_time_list = user_item_time_dict[
        user_id
    ]  #  [(item1, time1), (item2, time2)..]
    user_hist_items = set(
        [i for i, t in user_item_time_list]
    )  # 存在一个用户与某篇文章的多次交互， 这里得去重

    items_rank = {}
    for sim_u, wuv in sorted(
        u2u_sim[user_id].items(), key=lambda x: x[1], reverse=True
    )[:sim_user_topk]:
    ...
    items_rank = sorted(items_rank.items(), key=lambda x: x[1], reverse=True)[
        :recall_item_num
    ]
...
pickle.dump(
    user_recall_items_dict, open(save_path + "cold_start_items_raw_dict.pkl", "wb")
)
...
def cold_start_items(
    ...
    for user, item_list in tqdm(user_recall_items_dict.items()):
        ...
        for item, score in item_list:
            ...
            cold_start_user_items_dict[user].append(
                (item, score)
...
final_recall_items_dict_rank = combine_recall_results(
    user_multi_recall_dict, weight_dict, topk=150
)
