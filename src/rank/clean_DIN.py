...
def clean_collate_fn(batch, seq_max_len):
    """
...
class CleanDINRanker(BaseRanker):
    ...
    def train(self):
        ...
        if val_data is not None and len(val_data) > 0:
            ...
            val_loader = DataLoader(
                ...
                shuffle=False,
                collate_fn=partial(
                    clean_collate_fn, seq_max_len=self.config.din_seq_max_len
                ),
            )
        ...
        for epoch in range(self.config.epochs):
            ...
            for batch_idx, batch in tqdm(enumerate(train_loader), desc=f"Epoch {epoch}/{self.config.epochs}"):
